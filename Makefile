...
CFLAGS := -g -I$(PROJECT_INC_DIR) `pkg-config --cflags opencv` -std=c++11 
LDFLAG := -L$(PROJECT_LIB_DIR) `pkg-config --libs opencv` -lSQLiteCpp -lsqlite3 $(PROJECT_LIB_DIR)/libvl.so
